...
腾讯和火山翻译已从源码中移除。腾讯翻译君页面已提示其翻译 API 于 2025-04-15 关闭，当前网页端还要求腾讯验证码后才能获取翻译 token；火山翻译旧入口已返回 404，新入口则提示网站永久下线，因此没有保留不可运行的适配器。
...
3. 腾讯和火山翻译已从源码移除；若未来出现稳定、公开的官方 API，再考虑重新接入。
